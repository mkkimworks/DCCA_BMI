function FA = fcnFA(yMat, nLatents, varargin)

[estParams, LL] = fastfa(yMat', nLatents, 'typ', 'fa');
FA.estParams    = estParams;
FA.LL           = LL;
fcn_            = @(Y)fastfa_estep(Y', estParams);
FA.fcn          = @(Y)fcn_(Y).mean';
...
% 
% % fa.C: FA coordinate
% FA.beta                     = fa.C'/(fa.C*fa.C' + fa.psi); 
% FA.o                        = -FA.beta*fa.d;
% FA.fcn                      = @(Z)(FA.beta*Z' + FA.o)'; % latent variable
% FA.estParams                = fa;