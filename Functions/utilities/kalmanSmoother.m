...
Q = model.Q; % transition covariance
C = model.C; % emission matrix
R = model.R;  % emision covariance
...
llh(1) = logGauss(X(:,1),C*mu0,W);
for i = 2:n    
    [mu(:,i), V(:,:,i), Amu(:,i), P(:,:,i), llh(i)] = ...
        forwardUpdate(X(:,i), mu(:,i-1), V(:,:,i-1), A, Q, C, R);
end
llh = sum(llh);
% backward
nu = zeros(q,n);
...
function [mu1, V1, Amu, P, llh] = forwardUpdate(x, mu0, V0, A, Q, C, R)
k               = numel(mu0);
P               = A*V0*A'+Q;                                             % 13.88
PC              = P*C';
W               = C*PC+R;
